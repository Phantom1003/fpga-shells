...
class NexysA7MIGIODDR(depth : BigInt) extends GenericParameterizedBundle(depth) {
  require((depth<=0x8000000L),"Arty100TMIGIODDR supports upto 128 MB depth configuraton")
...
trait NexysA7MIGIOClocksReset extends Bundle {
  //inputs
...
class nexysa7mig(depth : BigInt)(implicit val p:Parameters) extends BlackBox
{
  require((depth<=0x8000000L),"arty100tmig supports upto 128 MB depth configuraton")

  val io = new NexysA7MIGIODDR(depth) with NexysA7MIGIOClocksReset {
    // User interface signals
  ...
  val migprj = """{<?xml version="1.0" encoding="UTF-8" standalone="no"?>
    <Project NoOfControllers="1">
      ...
      <Controller number="0">
        ...
        <mrWriteRecovery name="Write Recovery">3</mrWriteRecovery>
  ...
  // IP Core: nexys a7 mig
  val migprjname = """{/nexysa7mig.prj}"""
  val modulename = """nexysa7mig"""
